...
public class Carretera
{
    ...
    public static byte[] SerializarCarretera(Carretera carretera)
    {
        using (MemoryStream ms = new MemoryStream())
        {
            BinaryFormatter bf = new BinaryFormatter();
            bf.Serialize(ms, carretera);
            return ms.ToArray();
        }
    }

    public static Carretera DeserializarCarretera(byte[] bytes)
    {
        using (MemoryStream ms = new MemoryStream(bytes))
        {
            BinaryFormatter bf = new BinaryFormatter();
            return (Carretera)bf.Deserialize(ms);
        }
    }
